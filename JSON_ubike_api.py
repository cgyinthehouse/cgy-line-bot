...
def ubike_info(station):
    for x in range(0, len(datas["result"]["records"])):
        if datas["result"]["records"][x]["sna"] == station:
            info = str(
                    f'中文場站名稱:{datas["result"]["records"][x]["sna"]}\n' +
                    f'場站總停車格:{datas["result"]["records"][x]["tot"]}\n' +
                    f'場站目前車輛數{datas["result"]["records"][x]["sbi"]}\n' +
                    f'地址:{datas["result"]["records"][x]["ar"]}\n' +
                    f'場站是否暫停營運:{datas["result"]["records"][x]["act"]}')
            return info


if __name__ == "__main__":
    if sys.argv:
        msg = ubike_info(sys.argv[1])
        print(msg)
